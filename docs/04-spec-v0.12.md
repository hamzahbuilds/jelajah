...
## Addendum — additions requested during the build (all shipped in v0.12)

- **Bug fix — Plan ordering**: the day view sorted purely by time, so untimed
  activities snapped to the bottom no matter how they were moved and same-time
  items could flip. Display now honours the saved order (sort column) with
  auto events interleaved by time; drags dropped outside no longer leave a
  ghosted row.
- **Bug fix — map pins**: the rotated teardrop planted its visible tip ~4 px
  south of the anchored coordinate; icon geometry now anchors the tip exactly.
- **Security fix**: GET /participants was reachable by member accounts; now
  admin-only. A full audit of all routes confirmed payments/balances are
  strictly per-trip and members only see trips their participant belongs to.
- **My-spend peer tagging**: tag trip participants on a private item — each
  tagged person owes their equal share (optional "include me" divisor); tagged
  people see what they owe under "I owe"; the owner marks shares received;
  Owed-to-me / I-owe stat tiles. Admin still cannot see any of it.
- **Flight/stay participant tags**: auto events carry the booking's share
  participants; Plan shows 👥 names when a booking is not for everyone, and
  member dashboards only show the flights they are on.
- **Per-trip plan-edit permission**: "Members can edit the plan" toggle on
  People — members can then add/edit/reorder/strike activities (enforced
  server-side via canEditPlan()).
- **Activity categories platform-wide**: activities.category
  (sightseeing/food/transport/lodging/shopping/other) with icons, modal select,
  CSV template column, wizard Category+Price mapping (auto-categorised from the
  title when unmapped; ¥ prices → MYR estimates), MCP + AI suggestion support.

## Post-release patches
- **v0.12.1**: Gemini preset gemini-2.0-flash was retired by Google (Jun 2026,
  404) → preset now gemini-2.5-flash; provider errors surface the upstream
  message; thinking-model token budgets raised; wrangler.toml removed from
  delivery zips permanently.
- **v0.12.2**: Google's NEW AQ.-format API keys are rejected by Gemini's
  OpenAI-compat endpoint → any googleapis base URL now routes server-side to
  the NATIVE generateContent API (x-goog-api-key header; translation helpers
  in shared/assistant.ts, unit-tested). Both AIza and AQ. keys work.
